...
import { updateFortuneTeller } from '../services/supabaseService';
import { supabase } from '../../lib/supabase';

const EditFortuneTellerScreen = ({ navigation, route }) => {
  const { teller } = route.params;
  const [loading, setLoading] = useState(false);
  const [formData, setFormData] = useState({
    name: teller.name || '',
    profile_image: teller.profile_image || null,
    bio: teller.bio || '',
    experience_years: teller.experience_years?.toString() || '',
    specialties: teller.specialties || [],
    price_per_fortune: teller.price_per_fortune?.toString() || '',
    rank: teller.rank || 'başlangıç',
    is_available: teller.is_available !== undefined ? teller.is_available : true,
  });
  const [imageChanged, setImageChanged] = useState(false);

  // Uzmanlık alanları
  const specialtyOptions = [
    { id: 'kahve', name: 'Kahve Falı', icon: 'coffee' },
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (imageChanged && formData.profile_image && !formData.profile_image.startsWith('http')) {
      ...
      const { data, error } = await updateFortuneTeller(teller.id, updateData);

      if (error) throw error;

      Alert.alert(
        'Başarılı',
        'Falcı bilgileri başarıyla güncellendi!',
        [
          {
            text: 'Tamam',
      ...
      console.error('Falcı güncelleme hatası:', error);
      Alert.alert('Hata', 'Falcı güncellenirken bir hata oluştu: ' + error.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Text style={styles.headerTitle}>Falcı Düzenle</Text>
        <TouchableOpacity
        ...
        <View style={styles.section}>
          ...
          <View style={styles.statsContainer}>
            <View style={styles.statItem}>
              ...
              <Text style={styles.statValue}>{teller.rating?.toFixed(1) || '5.0'}</Text>
            </View>
            <View style={styles.statItem}>
              <Text style={styles.statLabel}>Toplam Fal</Text>
              <Text style={styles.statValue}>{teller.total_readings || 0}</Text>
            </View>
            <View style={styles.statItem}>
              <Text style={styles.statLabel}>Üyelik</Text>
              <Text style={styles.statValue}>
                {teller.created_at ? new Date(teller.created_at).toLocaleDateString('tr-TR') : 'Bilinmiyor'}
              </Text>
            </View>
          </View>
        </View>
...
export default EditFortuneTellerScreen; 